...
python main.py -data fmnist -m cnn -algo FedAvg -gr 100 -nc 58 -jr 0.25 -did 0 2>&1 | tee command1.log
...
for cdr_value in "${cdr_values[@]}"; do
    # Run the first command and store the output in a log file
    python main.py -data fmnist -m cnn -algo FedAvg -gr 100 -nc 58 -jr 0.25 -cdr "$cdr_value" -did 0 2>&1 | tee "command1_$cdr_value.log"

    # Save the output of the first command to a text file
    cat "command1_$cdr_value.log" > "NormalSelection_$cdr_value.txt"
    cat "saida.txt" > "NormalSelection_$cdr_value_\saida.txt"
    rm "saida.txt"

    # Run the second command and store the output in a log file
    python main.py -data fmnist -m cnn -algo FedAvg -gr 100 -nc 58 -ent True -cdr "$cdr_value" -did 0 2>&1 | tee "command2_$cdr_value.log"

    # Save the output of the second command to a text file
    cat "command2_$cdr_value.log" > "EntropySelection_$cdr_value.txt"
    cat "saida.txt" > "EntropySelection_$cdr_value_\saida.txt"
    rm "saida.txt"


    python main.py -data fmnist -m cnn -algo FedAvg -gr 100 -nc 58 -ba True -cdr "$cdr_value" -did 0 2>&1 | tee "command2_$cdr_value.log"

    # Save the output of the second command to a text file
    cat "command3_$cdr_value.log" > "BellowAverage_$cdr_value.txt"
    cat "saida.txt" > "BellowAverage_$cdr_value_\saida.txt"
    rm "saida.txt"
